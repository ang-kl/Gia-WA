...
describe('telemetry', () => {
  it('record is a no-op without redis', async () => {
    await expect(
      record({ task: 'classify', provider: 'groq', model: 'llama-3.1-8b-instant', usage: null }),
  ...
  it('dailySummary aggregates multiple calls', async () => {
    const redis = makeListRedis();
    init(redis);
    await record({ task: 'classify', provider: 'groq', model: 'llama-3.1-8b-instant',    usage: { prompt_tokens: 100, completion_tokens: 20 } });
    await record({ task: 'converse', provider: 'groq', model: 'llama-3.3-70b-versatile', usage: { prompt_tokens: 500, completion_tokens: 80 } });
  ...
  it('cost_usd sums to 0 for unknown model (no rates)', async () => {
    const redis = makeListRedis();
    init(redis);
    await record({ task: 'classify', provider: 'groq', model: 'unknown-model', usage: { prompt_tokens: 100, completion_tokens: 10 } });
    const s = await dailySummary();
